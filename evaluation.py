...
class DetectionMeasurement:
    ...
    def __init__(self, actuals, predictions, prediction_format, actuals_format='cvat', frame_numbers=None,
        ...
        self.results_df = pd.DataFrame(columns=['iou_threshold', 'frame_number', 'class_name',
                                                'actual', 'detection', 'confidence', 'TP', 'FP', 'FN'])
                ...
                if distances[frame_class]:
                    ...
                    for row, column in indices:
                        ...
                        if iou >= iou_threshold:
                            ...
                            self.results_df = self.results_df.append(pd.DataFrame({
                                'iou_threshold': iou_threshold,
                                'frame_number': frame_class[0],
                                'class_name': frame_class[1],
                                'actual': actual,
                                'detection': pred,
                                'confidence': preds[pred]['confidence'],
                                'TP': 1,
                                'FP': 0,
                                'FN': 0
...
'''
you know how to read the code if you skip codes above

actual is the groundtruth downloaded from google directory

pred is the prediction processed by darknet2maskrcnn.py

numbers in pred are str, we have to convert them into float

then run the DetectionMeasurement and get all metrics
one strange thing about this code is that it does not work on my laptop, something is wrong with multiprocessing, but it works in GCP, so run it on GCP

this DetectionMeasurement code is from Emily
'''
if __name__=='__main__':
    with open('xiong/traffic_video_GP010589_190720_0310_0440_90sec_calibrated_stable.json', 'r') as f:
        actual = json.load(f)

    with open('traffic_video_GOPR0589_190720_1324_1454_90sec_calibrated/pred.json', 'r') as f1:
        pred = json.load(f1)
